...
/**
 * Pseudo Trie tree implementation. It differs from a Trie tree in that it only holds one possible pattern (thus one branch),
 * and uses the DFA pattern used by RabbitMQ to evaluate wildcard filters.
 * Matching a category to a pattern is performed by walking the category components (separated by dots) and evaluating each
 * component against its counterpart in the pattern component chain.
 * If a pattern component is *, the category counterpart evaluates to true.
 * If a pattern component is #, it acts as though zero or more '*' components are present in the pattern at that point.
